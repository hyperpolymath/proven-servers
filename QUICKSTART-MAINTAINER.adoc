...
// Replace proven-servers, proven-servers, {{DEPS}} with actuals
= proven-servers — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining proven-servers for
...
| `$PREFIX/share/proven-servers/`
| Data files, assets

| `$PREFIX/share/doc/proven-servers/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/proven-servers/config.toml`

Fallback: `$HOME/.config/proven-servers/config.toml`
...
Or via OPSM: `opsm update proven-servers`
...
[source,bash]
----
just install --prefix=/opt/proven-servers-instance1 --config=/etc/proven-servers/instance1.toml
just install --prefix=/opt/proven-servers-instance2 --config=/etc/proven-servers/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/proven-servers/issues
