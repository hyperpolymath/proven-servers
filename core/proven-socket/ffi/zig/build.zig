...
//
// Build configuration for proven-socket FFI.
//
// Produces both a C-ABI-compatible shared library (libproven_socket.so /
// libproven_socket.dylib / proven_socket.dll) and a static library
// (libproven_socket.a) from the same source.
//
// Build targets:
...
// generated/abi/socket.h and the Idris2 ABI specifications at
// src/SocketABI/.
//
...
pub fn build(b: *std.Build) void {
    ...
    // -- Importable socket module for tests --
    ...
    const test_module = b.createModule(.{
        .root_source_file = b.path("test/socket_test.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
