...
//
// Build configuration for proven-ospf FFI.
//
// Produces both a C-ABI-compatible shared library (libproven_ospf.so /
// libproven_ospf.dylib / proven_ospf.dll) and a static library
// (libproven_ospf.a) from the same source.
//
// Build targets:
...
pub fn build(b: *std.Build) void {
    ...
    const lib_module = b.createModule(.{
        .root_source_file = b.path("src/ospf.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const ospf_module = b.createModule(.{
        .root_source_file = b.path("src/ospf.zig"),
        .target = target,
        .optimize = optimize,
    });
