...
//
// Build configuration for proven-ptp FFI.
//
// Produces both a C-ABI-compatible shared library (libproven_ptp.so /
// libproven_ptp.dylib / proven_ptp.dll) and a static library
// (libproven_ptp.a) from the same source.
//
// Build targets:
...
pub fn build(b: *std.Build) void {
    ...
    const lib_module = b.createModule(.{
        .root_source_file = b.path("src/ptp.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const ptp_module = b.createModule(.{
        .root_source_file = b.path("src/ptp.zig"),
        .target = target,
        .optimize = optimize,
    });
