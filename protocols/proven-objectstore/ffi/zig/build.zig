...
//
// Build configuration for proven-objectstore FFI.
...
pub fn build(b: *std.Build) void {
    ...
    const lib_module = b.createModule(.{
        .root_source_file = b.path("src/objectstore.zig"),
        .target = target,
        .optimize = optimize,
    });
    const shared_lib = b.addLibrary(.{
        .name = "proven_objectstore",
    ...
    const test_module = b.createModule(.{
        ...
        .target = target,
        .optimize = optimize,
        .imports = &.{.{ .name = "objectstore", .module = os_module }},
    });
