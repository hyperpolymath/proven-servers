...
//
// Build configuration for proven-rtsp FFI.
//
// Produces both a C-ABI-compatible shared library (libproven_rtsp.so /
// libproven_rtsp.dylib / proven_rtsp.dll) and a static library
// (libproven_rtsp.a) from the same source.
//
// Build targets:
...
pub fn build(b: *std.Build) void {
    ...
    const lib_module = b.createModule(.{
        .root_source_file = b.path("src/rtsp.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const rtsp_module = b.createModule(.{
        .root_source_file = b.path("src/rtsp.zig"),
        .target = target,
        .optimize = optimize,
    });
