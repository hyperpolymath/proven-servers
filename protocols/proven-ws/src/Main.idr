...
covering
printSep : IO ()
printSep = putStrLn (pack (Data.List.replicate 60 '-'))
...
demoFrames = do
  ...
  let clientFrame = MkFrame
        ...
        , payloadLength = 5
        , maskingKey = Just (the (Vect 4 Bits8) [0xAA, 0xBB, 0xCC, 0xDD])
  ...
  let bigControl = MkFrame
        ...
        , payloadLength = 200
        , maskingKey = Just (the (Vect 4 Bits8) [0x11, 0x22, 0x33, 0x44])
...
demoCloseCodes = do
  ...
  let codes : List CloseCode
      codes = [Normal, GoingAway, ProtocolError, UnsupportedData,
