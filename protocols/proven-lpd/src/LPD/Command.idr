...
||| Display a Bits8 as its decimal representation (helper for Show).
showHexByte : Bits8 -> String
showHexByte b = show (cast {to=Nat} b)

public export
Show CommandParseError where
  show EmptyCommand        = "Empty command (no data)"
  show (UnknownCommand c)  = "Unknown command code: 0x" ++ showHexByte c
  show (MissingQueueName c) = "Missing queue name for command 0x" ++ showHexByte c
