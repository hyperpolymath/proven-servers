...
//
// Build configuration for proven-loadbalancer FFI.
//
...
pub fn build(b: *std.Build) void {
    ...
    const lib_module = b.createModule(.{
        .root_source_file = b.path("src/loadbalancer.zig"),
        .target = target,
        .optimize = optimize,
    });

    const shared_lib = b.addLibrary(.{
        .name = "proven_loadbalancer",
    ...
    const test_module = b.createModule(.{
        .root_source_file = b.path("test/loadbalancer_test.zig"),
        .target = target,
        .optimize = optimize,
        .imports = &.{
            .{ .name = "lb", .module = lb_module },
        },
    });
