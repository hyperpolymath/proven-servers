...
//
// Build configuration for proven-snmp FFI.
//
// Produces both a C-ABI-compatible shared library (libproven_snmp.so /
// libproven_snmp.dylib / proven_snmp.dll) and a static library
// (libproven_snmp.a) from the same source.
//
// Build targets:
...
pub fn build(b: *std.Build) void {
    ...
    const lib_module = b.createModule(.{
        .root_source_file = b.path("src/snmp.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const snmp_module = b.createModule(.{
        .root_source_file = b.path("src/snmp.zig"),
        .target = target,
        .optimize = optimize,
    });
