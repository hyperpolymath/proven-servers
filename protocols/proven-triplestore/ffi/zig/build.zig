...
//
// Build configuration for proven-triplestore FFI.
...
pub fn build(b: *std.Build) void {
    ...
    const lib_module = b.createModule(.{
        .root_source_file = b.path("src/triplestore.zig"),
        .target = target,
        .optimize = optimize,
    });
    const shared_lib = b.addLibrary(.{
        .name = "proven_triplestore",
    ...
    const test_module = b.createModule(.{
        ...
        .target = target,
        .optimize = optimize,
        .imports = &.{.{ .name = "triplestore", .module = ts_module }},
    });
