...
//
// Build configuration for proven-pop3 FFI.
//
// Produces both a C-ABI-compatible shared library (libproven_pop3.so /
// libproven_pop3.dylib / proven_pop3.dll) and a static library
// (libproven_pop3.a) from the same source.
//
// Build targets:
...
pub fn build(b: *std.Build) void {
    ...
    const lib_module = b.createModule(.{
        .root_source_file = b.path("src/pop3.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const pop3_module = b.createModule(.{
        .root_source_file = b.path("src/pop3.zig"),
        .target = target,
        .optimize = optimize,
    });
