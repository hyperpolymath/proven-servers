...
enum class EncryptionType(val tag: Int) {
    ...
    companion object {
        fun fromTag(tag: Int): EncryptionType? = entries.find { it.tag == tag }
    }
}

/** PrincipalType matching the Idris2 ABI tags. */
enum class PrincipalType(val tag: Int) {
    NT_UNKNOWN(0),
    NT_PRINCIPAL(1),
    NT_SRV_INST(2),
    NT_SRV_HST(3),
    ...
    companion object {
        fun fromTag(tag: Int): PrincipalType? = entries.find { it.tag == tag }
    }
}

/** TicketFlag matching the Idris2 ABI tags. */
enum class TicketFlag(val tag: Int) {
    FORWARDABLE(0),
    FORWARDED(1),
    PROXIABLE(2),
    PROXY(3),
    ...
    companion object {
        fun fromTag(tag: Int): TicketFlag? = entries.find { it.tag == tag }
    }
}
...
enum class AuthState(val tag: Int) {
    INITIAL(0),
    TGT_OBTAINED(1),
    ...
    companion object {
        fun fromTag(tag: Int): AuthState? = entries.find { it.tag == tag }
    }
}

/** EncStrength matching the Idris2 ABI tags. */
enum class EncStrength(val tag: Int) {
    STRONG(0),
    MEDIUM(1),
    WEAK(2);

    companion object {
        fun fromTag(tag: Int): EncStrength? = entries.find { it.tag == tag }
    }
}

/** PreAuthType matching the Idris2 ABI tags. */
enum class PreAuthType(val tag: Int) {
    PA_ENC_TIMESTAMP(0),
    PA_ETYPE_INFO2(1),
    PA_FX_FAST(2),
    PA_FX_COOKIE(3);
    ...
    companion object {
        fun fromTag(tag: Int): PreAuthType? = entries.find { it.tag == tag }
    }
}
