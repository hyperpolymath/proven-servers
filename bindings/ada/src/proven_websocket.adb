-- SPDX-License-Identifier: MPL-2.0
-- (PMPL-1.0-or-later preferred; MPL-2.0 required for GNAT ecosystem)
-- Copyright (c) 2026 Jonathan D.A. Jewell (hyperpolymath) <j.d.a.jewell@open.ac.uk>
--
-- Implementation of safe wrappers for the proven-ws Ada bindings.

package body Proven_Websocket is

   function Safe_Create_Context return Proven_Error.Slot_Id is
   begin
      return Proven_Error.Check_Slot (Create_Context);
   end Safe_Create_Context;

   procedure Safe_Destroy_Context (Slot : Proven_Error.Slot_Id) is
   begin
      Destroy_Context (int (Slot));
   end Safe_Destroy_Context;

   procedure Safe_Open (Slot : Proven_Error.Slot_Id) is
   begin
      Proven_Error.Check_Status (Open_Connection (int (Slot)));
   end Safe_Open;

   procedure Safe_Close
     (Slot : Proven_Error.Slot_Id;
      Code : Close_Code)
   is
   begin
      Proven_Error.Check_Status
        (Close_Connection (int (Slot),
                           unsigned_short (Close_Code'Enum_Rep (Code))));
   end Safe_Close;

end Proven_Websocket;
