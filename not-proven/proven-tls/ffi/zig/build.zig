...
//
// Build configuration for proven-tls FFI.
...
pub fn build(b: *std.Build) void {
    ...
    const shared_lib = b.addLibrary(.{
        .name = "proven_tls", .root_module = lib_module, .linkage = .dynamic,
    });
    ...
    const static_lib = b.addLibrary(.{
        .name = "proven_tls", .root_module = static_mod, .linkage = .static,
    });
    ...
    const test_module = b.createModule(.{
        ...
        .target = target, .optimize = optimize,
        .imports = &.{ .{ .name = "tls", .module = tls_module } },
    });
